...
RevOps analyst's morning — checking the record for data-quality problems, enriching it with
firmographic context, scoring and routing it, drafting a rep brief and follow-up email, and
(for formal RFPs) drafting a proposal from pre-approved content. Every run is logged so the
Metrics Dashboard can show the aggregate business impact across runs.
...
| 5. Marketing Campaign Enrollment | *(Nurture-routed leads only)* Shows the simulated enrollment into a marketing-platform nurture campaign — closes the sales→marketing loop instead of leaving the lead stranded. |
...
Step numbers occasionally skip (e.g. straight from 4 to 6) — that's expected. Steps 5 and 8 are
conditional and only render for the scenarios they apply to.
...
| **Needs Human Review** | The rule-based score and the AI's read of the lead disagreed by more than the configured threshold — a guardrail, not a failure. A human makes the final call rather than the system guessing. |
...
**Before vs After** — pick any scenario and see a side-by-side comparison: the manual process
...
**Metrics Dashboard** — aggregate telemetry across every run you've triggered: total runs,
automation success rate, average time saved, percentage needing human review, and an
...
and firmographic enrichment are mocked interfaces — see the README's "Swappable for
production" section for how each would connect to a real system.
...
was. This reflects how proposal/pricing content should actually be gated in a real deployment,
...
**Something looks broken / a chart didn't render.**
